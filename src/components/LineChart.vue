...
export default defineComponent({
  ...
  methods: {
    ...
    genererChart() {
      ...
      const countobject1 = Object.fromEntries(
        this.nye_oversettelser.map(item => [item.dato, item.antall])
      );
      const countobject2 = Object.fromEntries(
        this.nye_forslag.map(item => [item.dato, item.antall])
      );
      const countobject3 = Object.fromEntries(
        this.antall_kommentarer.map(item => [item.dato, item.antall])
      );
      let oversettelser = [];
      let forslag = [];
      let kommentarer = [];
