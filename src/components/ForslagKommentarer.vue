<template>
  <v-card v-if="forslag">
    <v-card-title class="pr-2 d-flex align-center justify-space-between">
      <div>
        <span class="d-none d-sm-inline"
          >{{ $t("kommentar.forslag_fra") }} {{ forslag.brukernavn }}</span
        >
        <span class="d-sm-none"> {{ forslag.brukernavn }}</span>
      </div>
    ...
    <v-card-text>
      <div class="text-h5 font-weight-bold mb-3">
        <v-row no-gutters align="center">
          ...
          <v-col sm="5" cols="12" class="d-flex justify-sm-end">
            <v-btn
              color="primary"
              :href="
      ...
      <div
        v-if="
          forslag.eksisterende_definisjoner &&
          forslag.eksisterende_definisjoner.length > 0
        "
      >
        <span class="">
          <v-icon size="small" color="red">mdi-alert-circle</v-icon>
          {{ $t("forslag.eksisterende_definisjoner_varsel") }}:
        </span>
        <div
...
export default defineComponent({
  ...
  methods: {
    ...
    lenke() {
      let url =
        "https://baksida.jisho.no/forslag/" + this.forslag_id + "/kommentarer";
