      ...
      <v-col>
        ...
        >
          <template v-slot:[`item.admin`]="{ item }">
            <v-icon v-if="item.admin === 1" color="green" size="small">
              mdi-checkbox-marked-circle
            </v-icon>
            <v-icon v-else color="red" size="small"> mdi-close-circle</v-icon>
          </template>
          <template v-slot:[`item.sist_sett`]="{ item }">
            {{
              new Date(item.sist_sett).toLocaleDateString('nb-NO', {
                year: 'numeric',
                month: 'short',
                day: 'numeric',
              })
...
export default defineComponent({
  ...
  methods: {
    getUserData() {
      JishoDataService.getAllUsers().then(response => {
